...
function initDashboard() {
  ...
  window.setInterval(() => updateFromRealtimeSource({ rotateDevice: true }), DATA_REFRESH_MS);
...
function createFactoryScene(canvas, devices) {
  ...
  camera.lookAt(0, 0, 0);
  createCameraControls(canvas, camera);
...
function createCameraControls(canvas, camera) {
  const target = new THREE.Vector3(0, 0.42, -0.3);
  const offset = camera.position.clone().sub(target);
  const state = {
    radius: offset.length(),
    theta: Math.atan2(offset.x, offset.z),
    phi: Math.acos(offset.y / offset.length()),
    mode: null,
    lastX: 0,
    lastY: 0,
  };
  ...
  function updateCamera() {
    state.phi = clamp(state.phi, 0.28, Math.PI * 0.48);
    state.radius = clamp(state.radius, 4.6, 17);
    const sinPhiRadius = Math.sin(state.phi) * state.radius;
    camera.position.set(
      target.x + sinPhiRadius * Math.sin(state.theta),
      target.y + Math.cos(state.phi) * state.radius,
      target.z + sinPhiRadius * Math.cos(state.theta)
    );
  ...
  function pan(dx, dy) {
    const scale = state.radius / 520;
    const right = new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld, 0);
    const up = new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld, 1);
    target.addScaledVector(right, -dx * scale);
    target.addScaledVector(up, dy * scale);
    target.x = clamp(target.x, -4.6, 4.6);
    target.y = clamp(target.y, -0.6, 3.2);
    target.z = clamp(target.z, -3.6, 3.6);
    updateCamera();
  }
  ...
  function onPointerDown(event) {
    ...
    state.mode = event.button === 0 ? "rotate" : "pan";
    state.lastX = event.clientX;
    state.lastY = event.clientY;
    canvas.setPointerCapture(event.pointerId);
    canvas.style.cursor = state.mode === "rotate" ? "grabbing" : "move";
  }
  ...
  canvas.addEventListener(
    ...
    (event) => {
      event.preventDefault();
      state.radius *= Math.exp(event.deltaY * 0.001);
      updateCamera();
    },
    { passive: false }
  );
...
function updateCharts(charts, snapshot) {
  ...
  window.__dashboardBooted.statusChartOption = charts.status.getOption();
